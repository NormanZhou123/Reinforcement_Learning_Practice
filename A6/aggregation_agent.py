...
def agent_step(reward, state): # returns NumPy array, reward: floating point, this_observation: NumPy array
    ...
    current_state[0] = float(state[0] / 100.0)
    feature = np.zeros(1100)
    last_feature = np.zeros(1100)
    feature_list = tiles(iht, 1, current_state)
    last_feature_list = tiles(iht, 1, state1)
